// AlphaX CRM — Lead Entry Point list: the data-entry workbench.
// Smart Import (upload → auto-convert → dump into Lead Entry Point, Lead, or Smart
// Lead), status colors, bulk conversion to Lead for users who hold that
// permission.
frappe.listview_settings["AlphaX Lead Entry Point"] = {
    get_indicator(doc) {
        const map = {
            New: "blue",
            Contacted: "orange",
    ...
    onload(listview) {
        ...
        if (frappe.model.can_create("Lead")) {
            listview.page.add_actions_menu_item(__("Convert to Lead"), () => {
                ...
                frappe.confirm(__("Convert {0} entry point(s) to Leads?", [names.length]), () => {
                    frappe.call({
                        method: "alphax_crm.crm.lead_entry_point.bulk_convert_to_lead",
                        args: { names },
                        freeze: true,
...
const ALPHAX_IMPORT_TARGET_LABELS = {
    "AlphaX Lead Entry Point": __("Lead Entry Points — calling list, operator qualifies to Lead"),
...
function alphax_scan_and_confirm(listview, file_url, import_as, existing_dialog) {
    frappe.call({
        method: "alphax_crm.api.lead_import.scan_file",
        args: { file_url, import_as },
        freeze: true,
        freeze_message: __("Scanning file…"),
        callback: (r) => {
            if (existing_dialog) existing_dialog.hide();
            alphax_confirm_import(listview, file_url, r.message);
        },
    });
}
...
function alphax_confirm_import(listview, file_url, scan) {
    ...
    fields.push({
        ...
        label: __("Skip duplicates (matching email or mobile in Leads, Lead Entry Points, or Smart Leads)"),
        fieldtype: "Check",
        default: 1,
    });
    fields.push({
...
function alphax_show_summary(s) {
    const noun_map = {
        "AlphaX Lead Entry Point": __("entry points created."),
