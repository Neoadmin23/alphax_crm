"""Lead Entry Point (pre-lead) automation for AlphaX CRM.

Implements the sales process flow:
  Sales person creates a Lead Entry Point -> contacts customer -> sets a status.
  Each status maps to a behavior (configured in AlphaX Lead Entry Point Status):
...
The lead's owner is the entry point's owner (no re-assignment required), and the
...
def validate(doc, method=None):
    if not doc.get("status"):
        default = frappe.db.get_value("AlphaX Lead Entry Point Status", {"is_default": 1}, "name")
    ...
    if settings.get("activity_monitor_enabled", 1) and settings.get("capture_status_change", 1):
        if before and previous_status != doc.get("status") and doc.get("status"):
            try:
                ...
                record_activity("AlphaX Lead Entry Point", doc.name, f"Status \u2192 {doc.status}",
                                frappe.session.user, f"Status changed to {doc.status}")
            except Exception:
                log_error("prospect status activity")

    behavior = frappe.db.get_value("AlphaX Lead Entry Point Status", doc.get("status"), "behavior") or "None"
...
def _revert_status(doc, previous_status, error):
    ...
    (on_update runs post-save). Left as-is, the Lead Entry Point would show a
    ...
    fallback = (
        previous_status
        or frappe.db.get_value("AlphaX Lead Entry Point Status", {"is_default": 1}, "name")
        or "New"
    )
    message = str(error)
    ...
    if fallback != doc.get("status"):
        ...
        doc.status = fallback
    frappe.db.set_value("AlphaX Lead Entry Point", doc.name, updates, update_modified=False)
...
def _clear_conversion_failure(doc):
    if doc.get("conversion_failed"):
        frappe.db.set_value(
            "AlphaX Lead Entry Point", doc.name,
...
def bulk_convert_to_lead(names):
    ...
    converted, failed = [], []
    for name in names:
        doc = frappe.get_doc("AlphaX Lead Entry Point", name)
        if doc.get("lead"):
...
def _convert_to_lead(doc, settings):
    ...
    lead.add_comment("Comment", text=_("Created automatically from Lead Entry Point {0}.").format(doc.name))
    frappe.msgprint(
...
def _ensure_followup(doc):
    ...
    exists = frappe.db.exists(
        "ToDo",
        {"reference_type": "AlphaX Lead Entry Point", "reference_name": doc.name, "status": "Open"},
    )
    if exists:
        return
    todo = frappe.get_doc(
        {
            ...
            "reference_type": "AlphaX Lead Entry Point",
            "reference_name": doc.name,
            "description": _("Follow up with lead entry point {0} ({1}).").format(doc.prospect_name, doc.name),
            "priority": "Medium",
        }
    )
...
def _convert_via_smart_lead(doc, settings):
    """Lead Entry Point -> Smart Lead -> Lead. The Smart Lead's own save maps to the Lead."""
    ...
    if lead_name:
        ...
        frappe.db.set_value("AlphaX Lead Entry Point", doc.name,
