...
Territory, Industry Type, AlphaX Lead Entry Point Status, ...) and lets the user
...
Prospect since renamed to Lead Entry Point)
to the current schema, which added Link-typed fields to AlphaX Lead Entry Point
...
three targets are supported here:
    ...
    AlphaX Lead Entry Point   — calling list; operator qualifies to Lead later
    Lead              — created directly
    AlphaX Smart Lead — canonical entry; syncs itself into a Lead on insert

Wired from the AlphaX Lead Entry Point list view (alphax_lead_entry_point_list.js). Files
...
TARGET_DOCTYPES = ("AlphaX Lead Entry Point", "Lead", "AlphaX Smart Lead")
DEFAULT_TARGET = "AlphaX Lead Entry Point"
...
# company name is "company_name" on Lead Entry Point/Lead but "organization" on
...
NAME_ALIASES = (
    ...
    # doctype (e.g. a Lead-shaped export dropped onto a Lead Entry Point import) —
...
def _is_duplicate(data: dict) -> bool:
    ...
    email_field_by_doctype = {
        "Lead": "email_id",
        "AlphaX Lead Entry Point": "email_id",
...
def _execute(file_url, decisions, skip_duplicates, run_ai, user, notify=False,
    ...
    prospect_meta = frappe.get_meta("AlphaX Lead Entry Point")
    ...
    for idx, r in enumerate(rows, start=2):
        ...
        try:
            ...
            elif target == "AlphaX Lead Entry Point" and prospect_meta.has_field("import_batch"):
